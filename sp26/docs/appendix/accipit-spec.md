...
```
valuebinding   ::= 'let' <symbol> {':' type}? '=' {<binexpr> | <gep> | <fncall> | <alloca> | <load> | <store> | <phi>}
terminator     ::= <jmp> | <br> | <ret>
```

所有的指令具有两种形式：

- value binding，定义一个变量 `<symbol>`，并给它赋值，即将右侧指令的值赋给左侧 `<symbol>` 代表的变量。你可以选择对该变量进行类型标注。
...
或者类比 C 代码 `int *a = (int *)malloc(100 * sizeof(int))`， 对应 `let %a = alloca i32, 100`，只不过 alloca 分配的是栈空间，返回的是栈上的地址。 对于多维数组，你可以认为alloca在定义的时候将其“拍扁”了，将其视作一个一维数组。例如`int a[2][3][4]`可以被翻译为`let %a = alloca i32, 24`。
...
- 二维数组 `int g[][5]` 获取地址 `&g[x][y]`，可以使用 C 代码 `(int *)g + x * 5 + y`。
...
```
fncall ::= 'call' <global_ident> list({',' <value>})
```

##### 说明

fncall 指令进行函数调用，符号 `<global_ident>` 必须是被调用的函数，后跟一个参数列表。

##### 类型规则

如果被调用的函数 `<global_ident>` 是 `fn(T_1, T_2, ..., T_{n-1}) -> T_0` 类型，那么参数列表的参数必须依次为 `T_1` `T_2` ... `T_{n-1}` 类型，这条指令讲返回一个 `T_0` 类型的值。

假设 `<global_ident>` 没有参数，即为 `fn() -> T` 类型，那么可以不写参数列表，即 `let %ret = call <global_ident>`。

假设 `<global_ident>` 的返回值是 unit，即 `fn(T_1, T_2, ...) -> ()` 类型，那么返回值也为 unit 类型。

#### Phi Instruction

```
phi ::= 'phi' '[' <local_ident> ',' <local_ident> ']' {', '[' <local_ident> ',' <local_ident> ']'}+
```

##### 说明

phi 指令用于解决控制流汇合点的值选择问题，我们在 Lab 3 中实现的 Accipit IR 是借助 `alloc` `load` `store` 实现的 Partial-SSA 形式，如果你想迭代到彻底的 SSA 形式，就需要使用 phi 指令，更详细的介绍请看附录 [SSA](../appendix/ssa.md)。 phi 指令要求至少两个分支，并且每一对都是标签在前，变量在后。标签和变量显然都只能是局部的符号。
...
若为 true，跳转到第一个 `<local_label>` 标记的基本块起始处执行；
若为 false，跳转到第二个 `<local_label>` 标记的基本块起始处执行。

jmp 进行无条件跳转，跳转到 `<local_label>` 标记的基本块起始处执行。
...
```
plist ::= separated_list({<param_ident> ':' <type>}, ',')
fun   ::= 'fn' <global_ident> '(' <plist> ')' '->' <type> {';' | <body>}
```
...
```
global ::= <global_ident> ':' 'region' <type> ',' <int_const> {'=' '[' seperated_list(<value>, ',') ']'}?
```

##### 说明

声明全局变量 `<global_ident>`，变量具有 `<type>` 类型和 `<int_const>`长度，同时具有一个可选的初始化列表。与 `alloca` 相同，不论原始维度如何，全局变量定义与初始化视为对一维数组进行。

如果没有初始化列表，则默认进行零初始化。如果指定了初始化列表，那么其长度与声明长度应相符。

##### 类型规则

和 `alloca` 类似，`<type>` 是全局变量可存储的元素类型，`<global_ident>` 的类型是对应的指针类型 `<type> *`。
...
则 `@a` 为 `i32*` 类型，所指向的地址能存放 24 个 `i32` 类型的元素, 进行零初始化。

而
```
@b : region i32, 6 = [1, 2, 3, 4, 5, 6]
```
中的 `@b` 也为 `i32*` 类型，但是进行了所指定的初始化。
