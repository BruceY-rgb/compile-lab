...
class Environment():
    def __init__(self):
        ...
        self.size: int = 0
        self.trace: list[Ident] = []
        self._trace_entry: list[int] = []

    def push_trace(self, label: Ident):
        self.trace.append(label)
    
    def rollback_trace(self):
        while len(self.trace) > self._trace_entry[-1]:
            self.trace.pop()
        self._trace_entry.pop()

    def push_frame(self):
        self.frames.append({})
        env._trace_entry.append(len(env.trace))
    ...
    def allocate(self, size: int, init: list[int] = []) -> Ptr:
        if self.size + size > self.capacity:
            ...
            self.memory.extend([
                random.randint(1, 0xffff) for _ in range(size_to_extend)])
...
class BasicBlock(IRNode):
    ...
    def eval(self) -> int | UnitConst:
        env.push_trace(self.label)
        # SSA semantics: leading phi instructions in a basic block must appear
        # to execute simultaneously. Read all incoming values first, then bind
        # them. Otherwise mutually-referencing phis (e.g., swap idioms) yield
        # wrong results when evaluated sequentially.
        phi_results: list[tuple[Ident, Any]] = []
        first_non_phi = 0
        for inst in self.insts:
            if isinstance(inst, ValueBinding) and isinstance(inst.op, Phi):
                value = inst.op.eval()
                if inst.op.reduced_msg:
                    inst.reduced_msg = f"let {inst.name} = {inst.op.reduced_msg}"
                phi_results.append((inst.name, value))
                first_non_phi += 1
            else:
                break
        for name, value in phi_results:
            env.add_local(name, value)
        for idx in range(first_non_phi, len(self.insts)):
            inst = self.insts[idx]
...
@dataclass
class Phi:
    srcs: list[tuple[Ident, Ident]]
    line: int | None = None

    def __str__(self):
        return "phi [" + ", ".join(f"{src[0]}, {src[1]}" for src in self.srcs) + "]"

    def eval(self) -> Ident:
        if len(env.trace) < 2:
            raise SemanticError(
                "No previous label found for Phi instruction.")
        label = env.trace[-2]
        self.reduced_msg = f"Phi {''.join(f'[{src[0]}, {src[1].eval()}]' for src in self.srcs)} with {label} as the previous label."
        for src in self.srcs:
            if src[0] == label:
                return src[1].eval()
        raise SemanticError(
            f"Phi instruction {self} has no matching source for label {label}.")
...
class BaseTransformer(Transformer):
    ...
    @v_args(meta=True)
    def phi(self, meta, items):
        srcs = [(items[i], items[i+1]) for i in range(0, len(items), 2)]
        return Phi(srcs, meta.line)
...
accipit_grammar = """
    ...
    value_binding_untyped : "let" ident "=" (bin_expr | gep | fncall | alloca | load | store | phi)
    value_binding_typed : "let" ident ":" type "=" (bin_expr | gep | fncall | alloca | load | store | phi)
    ...
    load : "load" ident
    phi: "phi" "[" ident "," value "]" ("," "[" ident "," value "]")*
